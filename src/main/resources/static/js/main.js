...
async function addCookBook() {
    ...
    if (userDataJSON) {
        ...
        try {
            const response = await fetch("/v1/api/cookbook", {
                method: "POST",
                headers: {
...
async function deleteCookBook(cookBookId)  {
    ...
    const token = sessionStorage.getItem("token");
    try {
         const response = await fetch(`/v1/api/cookbook?id=${cookBookId}`, {
             method: "DELETE",
             headers: {
                 "Authorization": `Bearer ${token}`,
                 "Content-Type": "application/json"
...
async function getListOfCookBooks() {
    ...
    try {
        const response = await fetch(`/v1/api/cookbook?userId=${userId}`, {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
