...
async function deleteRecipe(recipeId) {
    ...
    const token = sessionStorage.getItem("token");
    try {
        const response = await fetch(`/api/v1/recipe?recipeId=${recipeId}`, {
            method: "DELETE",
            headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
        ...
        if (response.status === 204) {
            ...
            window.location.href = "/v1/cookbook?cookBookId=" + dataFromServer.cookBookId;
        } else {
...
async function getListOfRecipes() {
    ...
    try {
        const response = await fetch(`/api/v1/recipe?cookBookId=${dataFromServer.cookBookId}`, {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
